...
test.describe("SEO & Metadata", () => {
	test("home page should have correct metadata", async ({ page }) => {
		...
		// Title
		await expect(page).toHaveTitle(/moni/i);
	...
	test("savings & investing category page should have correct metadata", async ({
		page,
	}) => {
		await page.goto("/savings-investing");

		// Title should mention savings/investing
		await expect(page).toHaveTitle(/savings.*investing/i);

		// Canonical URL
		const canonical = page.locator('link[rel="canonical"]');
		await expect(canonical).toHaveAttribute("href", /savings-investing/);
	});

	test("calculator pages should have correct metadata", async ({ page }) => {
		const tool = savingsInvesting[0];
	...
	test("all calculator pages should have proper h1 headings", async ({
		page,
	}) => {
		const tool = savingsInvesting[0];
		await page.goto(tool.url);
