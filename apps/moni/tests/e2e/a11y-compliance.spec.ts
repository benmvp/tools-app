 ...
 * All calculator pages use the same template, so testing one per category validates all.
 *
 * Axe-core WCAG AA tags include color contrast validation, so no separate
 * contrast tests are needed.
 *
 * When adding new calculators: No new a11y tests needed unless calculator has unique UI.
 */
